...
def send_callback_lead_email(email_to, vendor_name, client_name, cc_emails=[], bcc_emails=[]):
    subject = f"Callback Request From {client_name.title()}! For Photography Services! Don't Miss Out!" if client_name else "You've Got a Callback Request! For Photography Services! Don't Miss Out!"
    context = {'user_name': vendor_name, 'client_name': client_name}

    send_email_with_backend(
        subject=subject,
        template_name="photographer/email/callback.html",
        context=context,
        email_info={
